...
class Controller:
    ...
    def __str__(self):
        ...
        BaseMana: {2}, RemainingMana: {3}, OverloadLocked: {4}, \
            OverloadOwed: {5}
        Hero: {6}
        Hand: {7}
        Board: {8}
        ...
        """.format(self.id, self.play_state,
                   self.base_mana, self.remaining_mana, self.overload_locked,
                   self.overload_owed, None, self.hand_zone, self.board_zone,
                   self.secret_zone, self.deck_zone.count)
